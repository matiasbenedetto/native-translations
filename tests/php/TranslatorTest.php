<?php
/**
 * Unit tests for Wpnt_Translator — the only caller of the WP 7.0 AI connector.
 *
 * The headline test is the bug #32 regression guard: when a site pins a model via
 * the `wpnt_model_preference` filter (the documented fix for a provider that
...
final class TranslatorTest extends TestCase {
	...
	protected function setUp(): void {
		...
		$this->translator = new Wpnt_Translator( new Wpnt_Translation_Store(), new Wpnt_Languages() );
	}
	...
	public function test_pinned_model_is_sent_to_the_connector(): void {
		Wpnt_Test_State::$filters['wpnt_model_preference'] = array( 'claude-opus-4-8' );
		...
		$this->assertSame(
			...
			Wpnt_Test_State::$last_request['models'] ?? null,
	...
	public function test_pinned_model_preserves_priority_order(): void {
		Wpnt_Test_State::$filters['wpnt_model_preference'] = array( 'claude-opus-4-8', 'claude-sonnet-4-6' );

		$this->translate();

		$this->assertSame(
			array( 'claude-opus-4-8', 'claude-sonnet-4-6' ),
			Wpnt_Test_State::$last_request['models']
		);
	}
	...
	public function test_blank_entries_in_preference_are_filtered_out(): void {
		Wpnt_Test_State::$filters['wpnt_model_preference'] = array( '', 'claude-opus-4-8', null );
		$this->translate();
		$this->assertSame( array( 'claude-opus-4-8' ), Wpnt_Test_State::$last_request['models'] );
	}
	...
	public function test_configured_settings_model_is_used_as_the_default(): void {
		...
		Wpnt_Test_State::$options['wpnt_settings'] = array( 'model' => 'claude-opus-4-8' );
		$this->translate();
		$this->assertSame( array( 'claude-opus-4-8' ), Wpnt_Test_State::$last_request['models'] );
	}
	...
	public function test_filter_overrides_the_configured_model(): void {
		Wpnt_Test_State::$options['wpnt_settings']         = array( 'model' => 'claude-opus-4-8' );
		Wpnt_Test_State::$filters['wpnt_model_preference'] = array( 'claude-sonnet-4-6' );
		$this->translate();
		$this->assertSame( array( 'claude-sonnet-4-6' ), Wpnt_Test_State::$last_request['models'] );
	}
	...
	public function test_recovers_from_model_not_available_404_by_retrying_with_an_advertised_model(): void {
		...
		$this->assertSame( 'Hola', $out );
		$this->assertCount( 2, Wpnt_Test_State::$requests, 'The connector must be called twice — initial 404 then one retry.' );
		// First call used the connector's auto-pick (no model pinned).
		$this->assertArrayNotHasKey( 'models', Wpnt_Test_State::$requests[0] );
		// Retry pinned the advertised fallback model.
		$this->assertSame( array( 'claude-opus-4-8' ), Wpnt_Test_State::$requests[1]['models'] );
	}
	...
	public function test_does_not_loop_forever_on_repeated_not_available_errors(): void {
		...
		$this->assertSame( 'wpnt_model_not_available', $out->get_error_code() );
		$this->assertSame( 404, $out->get_error_data()['status'] );
		$this->assertCount( 2, Wpnt_Test_State::$requests, 'At most one retry — no infinite loop.' );
	}
	...
	public function test_friendly_error_when_not_available_and_no_advertised_fallback(): void {
		...
		$this->assertSame( 'wpnt_model_not_available', $out->get_error_code() );
		$this->assertSame( 404, $out->get_error_data()['status'] );
		$this->assertCount( 1, Wpnt_Test_State::$requests, 'No retry when there is no advertised fallback.' );
	}
	...
	public function test_pinned_model_404_falls_back_to_an_other_advertised_model(): void {
		...
		$this->assertSame( 'Hola', $out );
		$this->assertSame( array( 'claude-opus-4-8' ), Wpnt_Test_State::$requests[0]['models'] );
		$this->assertSame( array( 'claude-sonnet-4-6' ), Wpnt_Test_State::$requests[1]['models'], 'Retry must skip the model already tried.' );
	}
	...
	public function test_temperature_is_omitted_by_default(): void {
		$this->translate();
		$this->assertArrayNotHasKey( 'temperature', Wpnt_Test_State::$last_request );
	}

	public function test_temperature_is_sent_when_opted_in(): void {
		Wpnt_Test_State::$filters['wpnt_temperature'] = 0.3;
		$this->translate();
		$this->assertSame( 0.3, Wpnt_Test_State::$last_request['temperature'] );
	}
	...
	public function test_connector_wp_error_is_propagated(): void {
		Wpnt_Test_State::$connector_result = new WP_Error( 'prompt_client_error', 'Bad Request (400)', array( 'status' => 400 ) );
		$out = $this->translate();
		$this->assertInstanceOf( WP_Error::class, $out );
	...
	public function test_no_model_error_is_mapped_to_friendly_unavailable(): void {
		...
		// (the #14 silent-failure shape) is mapped to the friendly wpnt_ai_unavailable.
		Wpnt_Test_State::$connector_result = new WP_Error(
		...
		$out = $this->translate();
		$this->assertInstanceOf( WP_Error::class, $out );
		$this->assertSame( 'wpnt_ai_unavailable', $out->get_error_code() );
		$this->assertSame( 503, $out->get_error_data()['status'] );
	}

	public function test_unavailable_when_no_usable_model(): void {
		Wpnt_Test_State::$is_supported = false; // capability probe fails.
		$out = $this->translate();
		$this->assertInstanceOf( WP_Error::class, $out );
		$this->assertSame( 'wpnt_ai_unavailable', $out->get_error_code() );
	}
	...
	public function test_can_generate_text_true_when_supported(): void {
		$this->assertTrue( Wpnt_Translator::can_generate_text() );
	}

	public function test_can_generate_text_false_when_no_model(): void {
		Wpnt_Test_State::$is_supported = false;
		$this->assertFalse( Wpnt_Translator::can_generate_text() );
	}

	public function test_can_generate_text_result_is_cached(): void {
		Wpnt_Translator::can_generate_text();
		$this->assertSame( '1', Wpnt_Test_State::$transients['wpnt_text_generation_supported'] );
	}

	public function test_test_connection_reports_success(): void {
		Wpnt_Test_State::$connector_result = 'Hola';
	...
	public function test_test_connection_reports_failure(): void {
		...
		$this->assertSame( 'wpnt_ai_unavailable', $res['code'] );
	}
	...
	public function test_empty_input_skips_the_connector(): void {
		...
		$this->assertSame( '   ', $out );
		$this->assertNull( Wpnt_Test_State::$last_request, 'Blank text must not call the connector.' );
	}

	public function test_surrounding_code_fence_is_stripped(): void {
		Wpnt_Test_State::$connector_result = "```html\n<p>Hola</p>\n```";
		$this->assertSame( '<p>Hola</p>', $this->translate() );
	}

	public function test_system_instruction_is_forwarded(): void {
		$this->translate();
		$this->assertSame( 'SYS', Wpnt_Test_State::$last_request['system'] );
	}
	...
	/** Seeds a registered wpnt_language taxonomy term so get_term_for_code() resolves. */
	private function seed_language_term( string $code, int $term_id ): void {
		Wpnt_Test_State::$terms[ $term_id ] = array(
			'term_id'  => $term_id,
			'taxonomy' => Wpnt_Languages::TAXONOMY,
			'slug'     => $code,
	...
	private function invoke_validate( string $type, int $source_id, string $to_code ) {
		$ref = new ReflectionMethod( Wpnt_Translator::class, 'validate_target' );
	...
	public function test_validate_target_rejects_non_original_source(): void {
		...
		$this->assertSame( 'wpnt_source_not_original', $result->get_error_code() );
	...
	public function test_validate_target_originals_only_applies_to_terms(): void {
		$this->seed_languages();
		$this->seed_language_term( 'es', 100 );
		Wpnt_Test_State::$terms[7] = array( 'term_id' => 7, 'taxonomy' => 'category', 'slug' => 'news', 'name' => 'News' );
	...
	public function test_validate_target_keeps_same_language_guard_for_originals(): void {
		...
		$this->seed_language_term( 'es', 100 );
		Wpnt_Test_State::$posts[5] = array( 'ID' => 5, 'post_type' => 'post' );
		Wpnt_Test_State::$object_terms[5][ Wpnt_Languages::TAXONOMY ] = array( 'es' );
		Wpnt_Test_State::$post_meta[5]['_wpnt_is_original'] = '1';

		// Original, but source language already equals the target.
		$result = $this->invoke_validate( 'post', 5, 'es' );
		$this->assertInstanceOf( WP_Error::class, $result );
		$this->assertSame( 'wpnt_same_language', $result->get_error_code() );
	}

	public function test_detect_language_returns_validated_code(): void {
		$this->seed_languages();
		Wpnt_Test_State::$posts[5] = array(
			'ID'           => 5,
			'post_type'    => 'post',
			'post_title'   => 'Hola mundo',
			'post_content' => 'Esto es un texto en español.',
		);
		Wpnt_Test_State::$connector_result = 'es';
		...
		$this->assertStringContainsString( 'classifier', Wpnt_Test_State::$last_request['system'] );
		$this->assertStringContainsString( 'en, es', Wpnt_Test_State::$last_request['system'] );
	}

	public function test_detect_language_normalises_noisy_model_reply(): void {
		$this->seed_languages();
		Wpnt_Test_State::$posts[5] = array( 'ID' => 5, 'post_type' => 'post', 'post_title' => 'Hi', 'post_content' => 'Hello there.' );
		// Model adds quotes / casing / trailing words — we take the first valid token.
		Wpnt_Test_State::$connector_result = "EN.\nThe language is English.";
	...
	public function test_detect_language_rejects_out_of_allowlist_code(): void {
		$this->seed_languages();
		Wpnt_Test_State::$posts[5] = array( 'ID' => 5, 'post_type' => 'post', 'post_title' => 'Bonjour', 'post_content' => 'Texte en français.' );
		Wpnt_Test_State::$connector_result = 'fr'; // Not an enabled language.

		$out = $this->translator->detect_language( 'post', 5 );
		$this->assertInstanceOf( WP_Error::class, $out );
		$this->assertSame( 'wpnt_detect_failed', $out->get_error_code() );
	}

	public function test_detect_language_rejects_unknown_reply(): void {
		$this->seed_languages();
		Wpnt_Test_State::$posts[5] = array( 'ID' => 5, 'post_type' => 'post', 'post_title' => '123', 'post_content' => '456' );
		Wpnt_Test_State::$connector_result = 'unknown';
	...
	public function test_detect_language_empty_content_does_not_call_connector(): void {
		$this->seed_languages();
		Wpnt_Test_State::$posts[5] = array( 'ID' => 5, 'post_type' => 'post', 'post_title' => '', 'post_content' => '' );

		$out = $this->translator->detect_language( 'post', 5 );
		$this->assertInstanceOf( WP_Error::class, $out );
		$this->assertSame( 'wpnt_detect_empty', $out->get_error_code() );
		$this->assertNull( Wpnt_Test_State::$last_request );
	}

	public function test_detect_language_unavailable_when_ai_off(): void {
		$this->seed_languages();
		Wpnt_Test_State::$is_supported = false;
		Wpnt_Test_State::$posts[5] = array( 'ID' => 5, 'post_type' => 'post', 'post_title' => 'Hi', 'post_content' => 'Hello.' );
	...
	public function test_detect_language_reuses_self_heal_on_model_not_available(): void {
		...
		$this->assertSame( 'en', $this->translator->detect_language( 'post', 5 ) );
		$this->assertCount( 2, Wpnt_Test_State::$requests );
	}
}
