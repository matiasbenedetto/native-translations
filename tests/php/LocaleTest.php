<?php
/**
 * Unit tests for Wpnt_Locale — the front-end locale switch (#62).
 *
 ...
 * and the `wpnt_switch_locale` veto filter. DB-free: view context + the
...
final class LocaleTest extends TestCase {
	...
	protected function setUp(): void {
		...
		$this->locale    = new Wpnt_Locale( $store, $languages );

		// Site default locale is English; the es_ES pack is installed.
		Wpnt_Test_State::$locale              = 'en_US';
		Wpnt_Test_State::$available_languages = array( 'es_ES' );
	}
	...
	private function seed_singular_post( int $id, string $code ): void {
		...
		Wpnt_Test_State::$object_terms[ $id ] = array( Wpnt_Languages::TAXONOMY => array( $code ) );
		Wpnt_Test_State::$queried_object      = get_post( $id );
		Wpnt_Test_State::$is_main_query       = true;
	}
	...
	public function test_spanish_singular_post_switches_locale(): void {
		...
		$this->assertSame( array( 'es_ES' ), Wpnt_Test_State::$switched_locales );
	}

	public function test_post_with_no_language_does_not_switch(): void {
		Wpnt_Test_State::$posts[8]        = array( 'ID' => 8, 'post_type' => 'post' );
		Wpnt_Test_State::$queried_object  = get_post( 8 );

		$this->locale->maybe_switch_locale();

		$this->assertSame( array(), Wpnt_Test_State::$switched_locales );
	}
	...
	public function test_locale_already_active_is_noop(): void {
		Wpnt_Test_State::$locale = 'es_ES'; // Already Spanish.
	...
	public function test_rest_context_does_not_switch(): void {
		...
		Wpnt_Test_State::$doing_ajax = true;
	...
	public function test_feed_and_robots_do_not_switch(): void {
		...
		Wpnt_Test_State::$is_feed = true;
		$this->locale->maybe_switch_locale();
		$this->assertSame( array(), Wpnt_Test_State::$switched_locales );

		Wpnt_Test_State::$is_feed   = false;
		Wpnt_Test_State::$is_robots = true;
		$this->locale->maybe_switch_locale();
		$this->assertSame( array(), Wpnt_Test_State::$switched_locales );
	}
	...
	public function test_non_main_query_does_not_switch(): void {
		...
		Wpnt_Test_State::$is_main_query = false;
	...
	public function test_category_archive_switches_to_term_language(): void {
		Wpnt_Test_State::$terms[20] = array( 'term_id' => 20, 'taxonomy' => 'category', 'name' => 'Noticias', 'slug' => 'noticias' );
		Wpnt_Test_State::$term_meta[20] = array( Wpnt_Translation_Store::META_LANGUAGE => 'es' );
		Wpnt_Test_State::$queried_object = get_term( 20 );
	...
	public function test_filter_can_veto_switch(): void {
		...
		Wpnt_Test_State::$filters['wpnt_switch_locale'] = false;
