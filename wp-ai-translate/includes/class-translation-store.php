 ...
 * the `_wpait_group` meta directly.
 *
...
class Wpait_Translation_Store {
	 ...
	 * carry their language via the `wpait_language` taxonomy instead.
	 */
	...
	public function set_language( string $object_type, int $object_id, string $code ) {
		...
		if ( 'term' === $object_type ) {
			...
			$result = wp_set_object_terms( $object_id, $code, Wpait_Languages::TAXONOMY, false );
	...
	public function link_translation( string $object_type, int $source_id, int $translation_id ) {
		...
		if ( '' === $code ) {
			return new WP_Error(
				'wpait_no_language',
				__( 'The translation has no language assigned.', 'wp-ai-translate' )
			);
		}
	...
	private function cache_key( string $object_type, string $group ): string {
		return 'wpait_group_' . $object_type . '_' . $group;
	}
	...
	public function on_set_object_terms( $object_id, $terms, $tt_ids, $taxonomy ): void {
		if ( Wpait_Languages::TAXONOMY !== $taxonomy ) {
			return;
		}
