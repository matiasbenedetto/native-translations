 ...
 * `{type,id,target}` payload; the {@see Wpnt_Queue::run_job()} callback resolves
...
class Wpnt_Queue {
	 ...
	 * @var string
	 */
	const HOOK = 'wpnt_translate_item';
	 ...
	 * @var string
	 */
	const DETECT_HOOK = 'wpnt_detect_item';
	 ...
	 * @var string
	 */
	const GROUP = 'native-translations';

	/**
	 * Default number of attempts (the first run plus retries) before a transient
	 * failure is reported as failed. Filterable via `wpnt_max_attempts`.
	 *
	 * @var int
	 */
	 ...
	 * @param Wpnt_Translator        $translator AI translator.
	 */
	public function __construct( Wpnt_Translation_Store $store, Wpnt_Languages $languages, Wpnt_Translator $translator ) {
		$this->store      = $store;
	...
	public function retry_job( int $action_id ) {
		if ( ! class_exists( 'ActionScheduler' ) || ! class_exists( 'ActionScheduler_Store' ) ) {
			return new WP_Error( 'wpnt_no_scheduler', __( 'The background scheduler is unavailable.', 'native-translations' ) );
		}
		$store  = ActionScheduler::store();
		$action = $store->fetch_action( $action_id );
		if ( ! $action || ! method_exists( $action, 'get_hook' ) || '' === $action->get_hook() ) {
			return new WP_Error( 'wpnt_unknown_action', __( 'That job no longer exists.', 'native-translations' ), array( 'status' => 404 ) );
		}
		...
		if ( self::HOOK === $hook ) {
			...
			return new WP_Error( 'wpnt_unknown_action', __( 'That job cannot be retried.', 'native-translations' ), array( 'status' => 400 ) );
		}

		if ( 'invalid' === $status ) {
			return new WP_Error( 'wpnt_retry_invalid', __( 'The job’s item or target is no longer valid.', 'native-translations' ), array( 'status' => 400 ) );
		}
	...
	public function cancel_job( int $action_id ) {
		...
		if ( self::HOOK !== $hook && self::DETECT_HOOK !== $hook ) {
			...
			return new WP_Error( 'wpnt_foreign_action', __( 'That job cannot be cancelled.', 'native-translations' ), array( 'status' => 400 ) );
		}

		try {
			$store->cancel_action( $action_id );
		} catch ( \Exception $e ) {
			return new WP_Error( 'wpnt_cancel_failed', __( 'The job could not be cancelled.', 'native-translations' ), array( 'status' => 500 ) );
		}
		return true;
	}

	/**
	 * Default batch size for {@see Wpnt_Queue::cancel_all()} pagination — how many
	 * actions per status are fetched and cancelled per query. Filterable via
	 * `wpnt_cancel_batch_size`.
	 *
	 * @var int
	 */
	 ...
	 * each batch fetches up to {@see Wpnt_Queue::CANCEL_BATCH_SIZE} actions and
	...
	public function cancel_all(): int {
		...
		$store      = ActionScheduler::store();
		$batch_size = max( 1, (int) apply_filters( 'wpnt_cancel_batch_size', self::CANCEL_BATCH_SIZE ) );
		$cancelled  = 0;
	...
	private function fail_or_retry( string $hook, array $payload, int $attempt, string $message ): void {
		$max = (int) apply_filters( 'wpnt_max_attempts', self::MAX_ATTEMPTS );
		...
		if ( $attempt < $max && function_exists( 'as_schedule_single_action' ) ) {
			...
			$delay     = (int) apply_filters( 'wpnt_retry_delay', 60, $attempt );
	...
	private function resolve_title( string $type, int $id ): string {
		...
		return sprintf( __( 'Item #%d', 'native-translations' ), $id );
	}
	...
	private function failure_message( int $action_id ): string {
		$generic = __( 'The job failed.', 'native-translations' );
