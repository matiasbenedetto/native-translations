 ...
 * Overview page under the WP Native Translations top-level menu.
 */
class Wpnt_Admin_List {

	/**
	 * Request var carrying the language filter value: a language code, or the
	 * special token `untranslated`.
	 */
	const QUERY_VAR = 'wpnt_lang';
	 ...
	 * Overview page slug.
	 */
	const OVERVIEW_SLUG = 'native-translations-overview';

	/**
	 * Translation Queue page slug (#96).
	 */
	const QUEUE_SLUG = 'native-translations-queue';
	 ...
	 */
	const HIDE_META = '_wpnt_exclude_from_overview';
	...
	public function register_hooks(): void {
		foreach ( Wpnt_Languages::OBJECT_TYPES as $post_type ) {
		...
		// Note: the /overview REST route is registered from Wpnt_Plugin (unconditionally,
	...
	public function add_column( array $columns ): array {
		...
		foreach ( $columns as $key => $label ) {
			if ( 'date' === $key ) {
				$out[ self::COLUMN ] = __( 'Language', 'native-translations' );
			}
			$out[ $key ] = $label;
		}
	...
	public function render_column( $column, $post_id ): void {
		...
		if ( '' === $code ) {
			echo '<span aria-hidden="true">—</span><span class="screen-reader-text">'
				. esc_html__( 'No language', 'native-translations' ) . '</span>';
			return;
		}
	...
	public function render_filter( $post_type ): void {
		if ( ! in_array( $post_type, Wpnt_Languages::OBJECT_TYPES, true ) ) {
			return;
		}
		...
		?>
		<label class="screen-reader-text" for="wpnt-lang-filter"><?php esc_html_e( 'Filter by language', 'native-translations' ); ?></label>
		<select name="<?php echo esc_attr( self::QUERY_VAR ); ?>" id="wpnt-lang-filter">
			<option value=""><?php esc_html_e( 'All languages', 'native-translations' ); ?></option>
			<option value="<?php echo esc_attr( self::UNTRANSLATED ); ?>" <?php selected( $current, self::UNTRANSLATED ); ?>>
				<?php esc_html_e( 'Untranslated', 'native-translations' ); ?>
			</option>
	...
	public function filter_query( $query ): void {
		...
		if ( ! $screen || 'edit' !== $screen->base || ! in_array( $screen->post_type, Wpnt_Languages::OBJECT_TYPES, true ) ) {
			return;
		}
	...
	public function filter_terms_query( $args, $taxonomies ) {
		...
		if ( self::UNTRANSLATED === $value ) {
			...
			$meta_query[] = array(
				'key'   => Wpnt_Translation_Store::META_LANGUAGE,
				'value' => $value,
			);
	...
	public function render_term_filter(): void {
		...
		$options = array(
			array( 'value' => '', 'label' => __( 'All languages', 'native-translations' ) ),
			array( 'value' => self::UNTRANSLATED, 'label' => __( 'Untranslated', 'native-translations' ) ),
		);
		...
		$data = array(
			...
			'current'  => $current,
			'label'    => __( 'Filter by language', 'native-translations' ),
			'options'  => $options,
		);
		?>
		<script>
		( function () {
			...
			if ( ! nav || document.getElementById( 'wpnt-term-lang-filter' ) ) {
				return;
			}
			var select = document.createElement( 'select' );
			select.id = 'wpnt-term-lang-filter';
	...
	private function skip_in_overview( string $type, int $id, bool $show_hidden ): bool {
		 ...
		 * @param int    $id   Object id.
		 */
		return (bool) apply_filters( 'wpnt_overview_skip', $skip, $type, $id );
	}
	 ...
	 * Hide / Unhide actions (REST → {@see Wpnt_Rest}).
	 *
	 * @param string $type   'post' | 'term'.
	 * @param int    $id     Object id.
	...
	private function count_in_language( string $code ): int {
		$query = new WP_Query(
			array(
				'post_type'      => Wpnt_Languages::OBJECT_TYPES,
				'post_status'    => 'any',
				'posts_per_page' => 1,
				'fields'         => 'ids',
				'ignore_sticky_posts' => true,
	...
	/**
	 * Registers the Overview REST route. Lives here (rather than in {@see Wpnt_Rest})
	...
	public function get_overview_payload( array $args ): array {
		...
		return array(
			...
			'ai_ok'         => Wpnt_Translator::can_generate_text(),
	 ...
	 * by the explicit {@see Wpnt_Translation_Store::META_IS_ORIGINAL} marker rather
		...
		foreach ( Wpnt_Languages::OBJECT_TYPES as $post_type ) {
			...
			foreach ( $ids as $post_id ) {
				...
				$out[]    = array(
					...
					'type'        => 'post',
					'id'          => $post_id,
					'title'       => get_the_title( $post_id ) ?: __( '(no title)', 'native-translations' ),
	...
	private function type_label( string $type, string $taxonomy, int $id ): string {
		if ( 'term' === $type ) {
			if ( 'category' === $taxonomy ) {
				return __( 'Category', 'native-translations' );
			}
			if ( 'post_tag' === $taxonomy ) {
				return __( 'Tag', 'native-translations' );
			}
			return $taxonomy;
		}
		...
		if ( 'page' === $post_type ) {
			return __( 'Page', 'native-translations' );
		}
		if ( 'post' === $post_type ) {
	...
	/**
	 * Registers the Overview submenu under the WP Native Translations top-level menu.
	 *
	 * @return void
	 */
	public function add_overview_page(): void {
		$this->overview_hook = (string) add_submenu_page(
			Wpnt_Admin_Settings::PAGE_SLUG,
			__( 'WP Native Translations Overview', 'native-translations' ),
			__( 'Overview', 'native-translations' ),
			'manage_options',
			self::OVERVIEW_SLUG,
	...
	public function enqueue_list_assets( $hook_suffix ): void {
		...
		$is_post_list = 'edit.php' === $hook_suffix
			&& 'edit' === $screen->base
			&& in_array( $screen->post_type, Wpnt_Languages::OBJECT_TYPES, true );
		...
		wp_enqueue_style(
			'wpnt-admin-list',
			WPNT_PLUGIN_URL . 'assets/css/admin-list.css',
			array(),
			WPNT_VERSION
		);
	}
	...
	public function enqueue_overview_assets( $hook_suffix ): void {
		...
		$asset_file = WPNT_PLUGIN_DIR . 'build/overview/index.asset.php';
		...
		wp_enqueue_script(
			'wpnt-overview',
			WPNT_PLUGIN_URL . 'build/overview/index.js',
			$asset['dependencies'],
			$asset['version'],
			true
		);
		...
		$style_file = WPNT_PLUGIN_DIR . 'build/overview/style-index.css';
		if ( file_exists( $style_file ) ) {
			wp_enqueue_style(
				'wpnt-overview',
				WPNT_PLUGIN_URL . 'build/overview/style-index.css',
				array( 'wp-components' ),
				$asset['version']
			);
		} else {
		...
		$config = array(
			'namespace' => Wpnt_Rest::NS,
			'languages' => array_values( $languages ),
			'aiOk'      => Wpnt_Translator::can_generate_text(),
			'perPage'   => self::PER_PAGE,
			'settingsUrl' => admin_url( 'admin.php?page=' . Wpnt_Admin_Settings::PAGE_SLUG ),
			'queueUrl'  => admin_url( 'admin.php?page=' . self::QUEUE_SLUG ),
		);

		wp_add_inline_script(
			'wpnt-overview',
			'window.wpntOverview = ' . wp_json_encode( $config ) . ';',
			'before'
		);

		wp_set_script_translations( 'wpnt-overview', 'native-translations' );
	}
	...
	public function render_overview(): void {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'WP Native Translations Overview', 'native-translations' ); ?></h1>
			<div id="wpnt-overview-app">
				<noscript>
					<?php esc_html_e( 'The WP Native Translations Overview requires JavaScript to display the translations table.', 'native-translations' ); ?>
				</noscript>
			</div>
		</div>
		<?php
	...
	/**
	 * Registers the Translation Queue submenu under the WP Native Translations top-level menu
	...
	public function add_queue_page(): void {
		$this->queue_hook = (string) add_submenu_page(
			Wpnt_Admin_Settings::PAGE_SLUG,
			__( 'Translation Queue', 'native-translations' ),
			__( 'Queue', 'native-translations' ),
			'manage_options',
			self::QUEUE_SLUG,
	...
	public function enqueue_queue_assets( $hook_suffix ): void {
		...
		$asset_file = WPNT_PLUGIN_DIR . 'build/queue/index.asset.php';
		if ( ! file_exists( $asset_file ) ) {
			// Build artifact missing; the page renders its <noscript> fallback only.
			return;
		}
		...
		wp_enqueue_script(
			'wpnt-queue',
			WPNT_PLUGIN_URL . 'build/queue/index.js',
			$asset['dependencies'],
			$asset['version'],
			true
		);
		...
		wp_add_inline_script(
			'wpnt-queue',
			'window.wpntQueue = ' . wp_json_encode( $config ) . ';',
			'before'
		);

		wp_set_script_translations( 'wpnt-queue', 'native-translations' );
	}
	...
	public function render_queue(): void {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Translation Queue', 'native-translations' ); ?></h1>
			<div id="wpnt-queue-app">
				<noscript>
					<?php esc_html_e( 'The Translation Queue requires JavaScript to display background jobs.', 'native-translations' ); ?>
				</noscript>
			</div>
		</div>
		<?php
	...
	private function truncation_message(): string {
		return sprintf(
			...
			__( 'Only the most recent %d items per type were scanned; the list may be incomplete. Use the by-language filter for older content.', 'native-translations' ),
			self::MAX_SCAN
		);
	}
	...
	private function language_column_html( string $type, int $id, string $code ): string {
		...
		$out  = '<span class="wpnt-language-cell">';
		if ( '' !== $flag ) {
			$out .= '<span class="wpnt-flag" aria-hidden="true">' . $flag . '</span>';
		}
		...
		if ( '' !== $source ) {
			$out .= ' <span class="description">'
				. sprintf(
					...
					esc_html__( 'translated from %s', 'native-translations' ),
					$source
				)
				. '</span>';
		}
	...
	private function original_source_link( string $type, int $id, string $code ): string {
		...
		$label = sprintf(
			...
			__( 'Edit the %s original', 'native-translations' ),
