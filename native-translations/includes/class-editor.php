 ...
 *
 * Both UIs are thin clients over the REST endpoints in {@see Wpnt_Rest}; this
...
class Wpnt_Editor {
	 ...
	 * the post's real language lives in the `wpnt_language` taxonomy, not here.
	 */
	 ...
	 */
	const TERM_FIELD_LANGUAGE    = 'wpnt_editor_language';
	const TERM_FIELD_IS_ORIGINAL = 'wpnt_editor_is_original';
	 ...
	 */
	public function __construct( Wpnt_Languages $languages, Wpnt_Translation_Store $store ) {
	 ...
	 *
	 * - `_wpnt_is_original` (the store's own original flag) is exposed to REST so
	 ...
	 * - `_wpnt_editor_language` is a staging field the language picker writes into;
	...
	public function register_post_meta_fields(): void {
		...
		foreach ( Wpnt_Languages::OBJECT_TYPES as $post_type ) {
			register_post_meta(
				$post_type,
				Wpnt_Translation_Store::META_IS_ORIGINAL,
				array(
					'type'              => 'boolean',
					'single'            => true,
					'show_in_rest'      => true,
	 ...
	 *
	 * The picker stages its choice in `_wpnt_editor_language`; here we consume that
	 * staging value and route it through {@see Wpnt_Translation_Store::set_language()}
	 ...
	 * {@see Wpnt_Translator::can_generate_text()}).
	 *
	 * @return bool
	 */
	private function ai_available(): bool {
		return Wpnt_Translator::can_generate_text();
	}
	...
	public function enqueue_panel(): void {
		...
		if ( ! $screen || ! in_array( $screen->post_type, Wpnt_Languages::OBJECT_TYPES, true ) ) {
			return;
		}

		$asset_file = WPNT_PLUGIN_DIR . 'build/editor-panel/index.asset.php';
		...
		wp_enqueue_script(
			'wpnt-editor-panel',
			WPNT_PLUGIN_URL . 'build/editor-panel/index.js',
			$asset['dependencies'],
			$asset['version'],
			true
		);
		...
		wp_add_inline_script(
			'wpnt-editor-panel',
			'window.wpntEditor = ' . wp_json_encode( $this->base_context() ) . ';',
			'before'
		);

		wp_set_script_translations( 'wpnt-editor-panel', 'native-translations' );
	}
	...
	public function enqueue_term_assets( $hook_suffix ): void {
		...
		wp_enqueue_script(
			'wpnt-term-panel',
			WPNT_PLUGIN_URL . 'assets/js/term-panel.js',
		...
		wp_add_inline_script(
			'wpnt-term-panel',
			'window.wpntTerm = ' . wp_json_encode( $context ) . ';',
			'before'
		);

		wp_set_script_translations( 'wpnt-term-panel', 'native-translations' );
	}
	...
	public function render_term_box( $term, $taxonomy ): void {
		...
		?>
		<tr class="form-field">
			<th scope="row"><?php esc_html_e( 'Translations', 'native-translations' ); ?></th>
			<td>
				<div id="wpnt-term-panel" data-term-id="<?php echo esc_attr( (string) $term->term_id ); ?>">
					<p class="description"><?php esc_html_e( 'Loading translations…', 'native-translations' ); ?></p>
				</div>
			</td>
		</tr>
		<?php
