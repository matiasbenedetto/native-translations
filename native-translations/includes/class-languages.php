...
/**
 * Owns the `wpnt_language` taxonomy and keeps its terms in sync with the
 * configured language list (S4).
 */
class Wpnt_Languages {

	const TAXONOMY = 'wpnt_language';

	const META_LOCALE = '_wpnt_locale';
	const META_CODE   = '_wpnt_code';
	const META_NATIVE = '_wpnt_native_name';
	const META_FLAG   = '_wpnt_flag';
	...
	public function register_hooks(): void {
		...
		add_action( 'add_option_' . Wpnt_Admin_Settings::OPTION, array( __CLASS__, 'flush_index' ) );
		add_action( 'update_option_' . Wpnt_Admin_Settings::OPTION, array( __CLASS__, 'flush_index' ) );
	}
	...
	public static function flag_html( string $flag, array $args = array() ): string {
		...
		if ( self::is_region_code( $flag ) ) {
			...
			return sprintf(
				'<img class="wpnt-flag-img%s" src="%s" alt="%s" loading="lazy" decoding="async" />',
		...
		return '<span class="wpnt-flag-emoji' . esc_attr( $extra_class ) . '">' . esc_html( $flag ) . '</span>';
	}
	...
	public function install_language_pack( string $locale ) {
		if ( '' === $locale || ! in_array( $locale, $this->configured_locales(), true ) ) {
			return new WP_Error(
				'wpnt_invalid_locale',
				__( 'That locale is not one of the site’s configured languages.', 'native-translations' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( false === $result ) {
			return new WP_Error(
				'wpnt_install_failed',
				__( 'The language pack could not be installed.', 'native-translations' ),
				array( 'status' => 500 )
			);
		}
		return true;
