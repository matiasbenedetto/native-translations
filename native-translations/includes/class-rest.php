<?php
/**
 * REST API: namespace `native-translations/v1`.
 *
...
class Wpnt_Rest {
	...
	const NS = 'native-translations/v1';
	 ...
	 * @param Wpnt_Queue             $queue      Background translation queue.
	 */
	public function __construct( Wpnt_Translation_Store $store, Wpnt_Languages $languages, Wpnt_Translator $translator, Wpnt_Queue $queue ) {
		$this->store      = $store;
	...
	public function handle_retry_job( WP_REST_Request $request ) {
		...
		if ( $desc && '' !== $desc['type'] && $desc['id'] > 0 ) {
			...
			if ( is_wp_error( $cap ) && 'wpnt_forbidden' === $cap->get_error_code() ) {
				return $cap;
			}
		}
	...
	public function handle_overview_visibility( WP_REST_Request $request ) {
		Wpnt_Plugin::instance()->admin_list()->set_overview_hidden(
			(string) $request->get_param( 'type' ),
			(int) $request->get_param( 'object_id' ),
	...
	public function handle_delete( WP_REST_Request $request ) {
		...
		if ( ! $trashed ) {
			return new WP_Error( 'wpnt_delete_failed', __( 'The translation could not be trashed.', 'native-translations' ), array( 'status' => 500 ) );
		}
	...
	private function validate_enabled_language( string $code ) {
		...
		return new WP_Error(
			...
			__( 'The language is not a configured, enabled language.', 'native-translations' ),
			array( 'status' => 400 )
		);
	}
	...
	private function forbidden(): WP_Error {
		return new WP_Error(
			'wpnt_forbidden',
			__( 'You are not allowed to translate this item.', 'native-translations' ),
	...
	private function not_found(): WP_Error {
		return new WP_Error(
			'wpnt_no_source',
			__( 'The requested item was not found.', 'native-translations' ),
			array( 'status' => 404 )
		);
	}
}
